...
void publish_points(T &new_pc, const sensor_msgs::PointCloud2 &old_msg) {
    ...
    // pc_new_msg.header.stamp.sec -= 28800;
    // pc_new_msg.header.stamp.nsec = (new_pc->points[0].timestamp - int(new_pc->points[0].timestamp)) * 1e9;
    // float time = pc_new_msg.header.stamp.sec + pc_new_msg.header.stamp.nsec*1.0/1e9;
    // std::cout << "point_time" << std::fixed << std::setprecision(0) << time  << std::endl;
    // pc_new_msg.header.stamp = ros::Time::now();
    // pc_new_msg.header.stamp.nsec = ros::Time::now().toNSec() - ros::Time::now().toSec()*1e9;
    // pc_new_msg.header.stamp.nsec = (new_pc->points[0].timestamp - int(new_pc->points[0].timestamp)) * 1e9;
    // int time = pc_new_msg.header.stamp.sec;
    // std::cout << "point_time_" << std::fixed << std::setprecision(0) << time  << std::endl;
...
void add_time(const typename pcl::PointCloud<T_in_p>::Ptr &pc_in,
    ...
    // to new pointcloud
    int valid_point_id = 0;
    // std::cout << "point0_timestamp" << std::fixed << std::setprecision(0) << pc_in->points[0].timestamp << std::endl;
    for (int point_id = 0; point_id < pc_in->points.size(); ++point_id) {
        if (has_nan(pc_in->points[point_id]))
            continue;
        // 跳过nan点
        pc_out->points[valid_point_id++].time = float(pc_in->points[point_id].timestamp - pc_in->points[0].timestamp);
        // std::cout << "lidar_timestamp" << pc_in->points[point_id].timestamp-1670940000 << std::endl;
    }
}
...
//             subRobosensePC = nh.subscribe("/hesai/pandar_points", 1, hesaiHandler_XYZI);
//         } else if (std::strcmp("XYZIRT", argv[1]) == 0) {
//             subRobosensePC = nh.subscribe("/hesai/pandar_points", 1, hesaiHandler_XYZIRT);
//         } else {
//             ROS_ERROR(argv[1]);
