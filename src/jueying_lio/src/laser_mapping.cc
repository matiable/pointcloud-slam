...
void LaserMapping::Reset() {
    ...
    scan_down_body_.reset(new PointCloudType());
    scan_undistort_.reset(new PointCloudType());
    scan_down_world_.reset(new PointCloudType());
    // esekf init
    std::vector<double> epsi(23, 0.001);
    kf_.init_dyn_share(
        get_f, df_dx, df_dw,
...
bool LaserMapping::LoadParams(ros::NodeHandle &nh) {
    ...
    if (lidar_type == 1) {
        ...
        preprocess_->SetLidarType(LidarType::RSLIDAR);
        LOG(INFO) << "Using Robosense Lidar";
    } else if (lidar_type == 5) {
        preprocess_->SetLidarType(LidarType::LIVOX_STD);
        LOG(INFO) << "Using Livox Lidar pc2 format";
    } else {
...
bool LaserMapping::LoadParamsFromYAML(const std::string &yaml_file) {
    ...
    try {
        ...
        map_file_path_ = yaml["map_file_path"].as<std::string>();
...
void LaserMapping::SubAndPubToROS(ros::NodeHandle &nh) {
    ...
    std::string lidar_topic, imu_topic, reset_flag_topic;
    nh.param<std::string>("common/lid_topic", lidar_topic, "/livox/lidar");
    nh.param<std::string>("common/imu_topic", imu_topic, "/livox/imu");
    reset_flag_topic = "jueying_lio/reset";
    ...
    sub_reset_flag_ = nh.subscribe<std_msgs::Bool>(reset_flag_topic, 10,
                                                   [this](const std_msgs::Bool::ConstPtr &msg) { ResetCallBack(msg); });
...
void LaserMapping::ResetCallBack(const std_msgs::Bool::ConstPtr &msg) {
    if (msg->data) {
        std::cout << "Reset LaserMapping" << std::endl;
        this->Reset();
    }
}
    ...
    if (pcd_save_en_) {
        *pcl_wait_save_ += *laserCloudWorld;
        ...
        if (pcl_wait_save_->size() > 0 && pcd_save_interval_ > 0 && scan_wait_num >= pcd_save_interval_) {
            pcd_index_++;
            std::string all_points_dir(std::string(std::string(map_file_path_) + "/jueying_") +
...
void LaserMapping::Finish() {
    ...
    if (pcl_wait_save_->size() > 0 && pcd_save_en_) {
        std::string file_name = std::string("jueying.pcd");
        std::string all_points_dir(std::string(std::string(map_file_path_)) + "/" + file_name);
        pcl::PCDWriter pcd_writer;
        LOG(INFO) << "Map saved to " << all_points_dir;
        pcd_writer.writeBinary(all_points_dir, *pcl_wait_save_);
    }
