...
void PointCloudPreprocess::Process(const sensor_msgs::PointCloud2::ConstPtr &msg, PointCloudType::Ptr &pcl_out) {
    switch (lidar_type_) {
        ...
        case LidarType::RSLIDAR:
            RslidarHandler(msg);
            break;
        
        case LidarType::LIVOX_STD:
            LivoxHandler(msg);
            break;
...
void PointCloudPreprocess::LivoxHandler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    ...
    for (int i = 0; i < pl_orig.points.size(); i++) {
        ...
        added_pt.normal_y = 0;
        added_pt.normal_z = 0;
        added_pt.curvature = (pl_orig.points[i].timestamp - pl_orig.points[0].timestamp) * time_scale_;  // curvature unit: ms
...
void PointCloudPreprocess::RslidarHandler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    cloud_out_.clear();
    cloud_full_.clear();
