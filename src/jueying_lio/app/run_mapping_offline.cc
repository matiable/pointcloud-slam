...
void SigHandle(int sig) {
    jueying_lio::options::FLAG_EXIT = true;
...
int main(int argc, char **argv) {
    ...
    auto laser_mapping = std::make_shared<jueying_lio::LaserMapping>();
    ...
    for (const rosbag::MessageInstance &m : rosbag::View(bag)) {
        ...
        if (jueying_lio::options::FLAG_EXIT) {
            break;
        }
    }
    ...
    /// print the fps
    double fps = 1.0 / (jueying_lio::Timer::GetMeanTime("Laser Mapping Single Run") / 1000.);
    ...
    jueying_lio::Timer::PrintAll();
    jueying_lio::Timer::DumpIntoFile(FLAGS_time_log_file);
