...
public:
    ...
    TransformFusion()
    {
        ...
        subLaserOdometry = nh.subscribe<nav_msgs::Odometry>("jueying_slam/mapping/odometry", 5, &TransformFusion::lidarOdometryHandler, this, ros::TransportHints().tcpNoDelay());
        subImuOdometry   = nh.subscribe<nav_msgs::Odometry>(odomTopic+"_incremental",   2000, &TransformFusion::imuOdometryHandler,   this, ros::TransportHints().tcpNoDelay());

        pubImuOdometry   = nh.advertise<nav_msgs::Odometry>(odomTopic, 2000); 
        pubImuPath       = nh.advertise<nav_msgs::Path>    ("jueying_slam/imu/path", 1);
    }
...
public:
    ...
    IMUPreintegration()
    {
        ...
        correctionNoise = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 1, 1, 1, 1, 1, 1).finished());
    ...
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        while (!imuQueOpt.empty())
        {
            ...
            {
                double dt = (lastImuT_opt < 0) ? (1.0 / float(imuFrequency)) : (imuTime - lastImuT_opt);
                if(dt > 0.01)
                {
                    dt = 1.0 / float(imuFrequency);
                }
        ...
        ++ResetCount;
        ResetCount%=4;
        if((odomMsg->pose.covariance[0]>0.15||odomMsg->pose.covariance[2]>0.3)&&location_initialied){

            prevStateOdom=gtsam::NavState(curPose, gtsam::Vector3(0,0,0));
            cout<<"Reset IMU"<<endl;
        }
        if(odomMsg->pose.covariance[0]<0.15&& odomMsg->pose.covariance[2]<0.15&&location_initialied==false){
    ...
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        //cout<<"current time "<< imuTime << " last time " << lastImuT_imu << " dt "<<dt<<endl;
        ...
        //cout<<"linear acc  x "<<thisImu.linear_acceleration.x<<" linear acc  y "<< thisImu.linear_acceleration.y<<" linear acc z "<<thisImu.linear_acceleration.z<<endl;
        //cout<<"linear x "<<currentState.velocity().x()<<" linear y "<< currentState.velocity().y()<<" linear z "<<lidarPose.translation().z()<<endl;
    }
};
